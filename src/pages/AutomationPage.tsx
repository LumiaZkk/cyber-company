...
import { buildAutomationScenario } from "../features/automation/recommendations";
import { useCompanyStore } from "../features/company/store";
import { gateway, type CronJob, type CronListResult } from "../features/backend";
...
export function AutomationPage() {
  ...
  }, [activeCompany?.id, companyAgentIdsKey]);
  ...
  const toggleCronStatus = async (job: CronJob) => {
    try {
      ...
      toast.error("状态更新失败", e.message || String(e));
    }
  };
              ...
              <div className="flex gap-4">
                <label className="block text-sm w-1/3">
                  ...
                  <select
                    ...
                    onChange={(e) => setDraftScheduleType(e.target.value as any)}
                    className="w-full rounded-lg border border-slate-200 px-3 py-2 outline-none transition focus:border-indigo-500"
                  >
